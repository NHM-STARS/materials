...
We can also "assign" values to objects, i.e. give them names. Let's mak an object called `x`.
```{r}
x <- 1:10
```
How do we "see" x?
...
The Console vs. The Script
 CTRL - 1 vs CTRL - 2
? or ?? or help() or help.start() to get help
 or google it
> is the prompt
...
Install the following packages (`dplyr`, `tidyr`, `ggplot2` and `gridExtra`), then load them as below.
...
Sometimes you won't have a `.Rmd` file. In that case you can set the working directory really simply by going to the Session menu in RStudio, then picking Choose Working Directory and choose the right folder.

You can check this worked using
```{r}
list.files()
```
This will print a list of the file names in your working directory.

## Importing data
Once you've set the working directory to the correct folder, it's easy to read in data! 
