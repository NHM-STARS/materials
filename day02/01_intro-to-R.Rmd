...
To make things easier, a lot of the packages we will use have been bundled up into one package called `tidyverse`. We can install all of the packages in one go using `install.packages("tidyverse")`
...
Note that you'll get some warnings when you do this. These just tell you that R already has some functions with the same names as those in `dplyr` and `tidyr`, and that from now on it will use the `dplyr` and `tidyr` versions by default. This is fine!
...
### Importing data
Once you've set the working directory to the correct folder, it's easy to read in data. We'll play with the `compensation.csv` dataset today.
